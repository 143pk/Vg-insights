...
export function renderProgressModal(): string {
  ...
  return `
    <div id="modal-progress" class="fixed inset-0 z-50 hidden items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-fade-in">
      <div class="w-full max-w-xl rounded-3xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-2xl overflow-hidden flex flex-col max-h-[85vh]">
        ...
        <div class="p-4 sm:p-5 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between bg-slate-50/50 dark:bg-slate-800/50">
          <div class="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p class="text-xs text-slate-500 dark:text-slate-400">Track topic completion & PYQ accuracy</p>
            </div>
          </div>
        ...
        <div class="p-5 overflow-y-auto space-y-6">
          ...
          <div class="grid grid-cols-2 gap-4">
            ...
            <div class="p-4 rounded-2xl bg-emerald-50/50 dark:bg-emerald-950/30 border border-emerald-500/20 space-y-1">
              <span class="text-xs font-semibold text-emerald-700 dark:text-emerald-400 uppercase">Topics Completed</span>
              <div class="text-2xl font-extrabold text-slate-900 dark:text-slate-100">
                ${completedCount} <span class="text-xs text-slate-400 font-normal">/ ${totalTopics}</span>
              </div>
              <div class="w-full h-2 bg-emerald-200 dark:bg-emerald-900 rounded-full overflow-hidden mt-2">
                <div class="h-full bg-emerald-500 rounded-full transition-all duration-500" style="width: ${percentage}%"></div>
              </div>
            </div>

            <div class="p-4 rounded-2xl bg-blue-50/50 dark:bg-blue-950/30 border border-blue-500/20 space-y-1">
              <span class="text-xs font-semibold text-blue-700 dark:text-blue-400 uppercase">PYQ Accuracy</span>
              <div class="text-2xl font-extrabold text-slate-900 dark:text-slate-100">
                ${pyqAccuracy}%
              </div>
              <div class="text-xs text-slate-500 dark:text-slate-400">
                ${correctPYQs} correct out of ${totalPYQsAttempted} attempted
              </div>
            </div>
              ...
              return `
                <div class="p-3.5 rounded-2xl bg-slate-50 dark:bg-slate-800/50 border border-slate-200/60 dark:border-slate-700/60 space-y-2">
                  <div class="flex items-center justify-between text-xs font-bold">
                    ...
                    <span class="text-slate-500">Class 11 & 12</span>
                  </div>
                </div>
              `;
            }).join('')}
